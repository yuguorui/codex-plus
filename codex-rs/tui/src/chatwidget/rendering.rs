...
impl ChatWidget {
    pub(crate) fn as_renderable(&self) -> RenderableItem<'_> {
        ...
        flex.push(/*flex*/ 0, bottom_pane_renderable);
    ...
    pub(crate) fn bottom_pane_renderable<'a>(
        ...
        if self.external_writer_view && !self.bottom_pane.has_active_view() {
                ...
                .is_some()
            {
                ...
                self.ambient_pet_composer_reserved_cols(
                    self.last_rendered_width.get().unwrap_or(u16::MAX),
                )
            };
            self.bottom_pane
    ...
    pub(super) fn active_cell_hyperlink_lines_with(
        ...
        let cells = self
            ...
            .chain(workflow_cell.map(|cell| cell as &dyn HistoryCell))
            .chain(
