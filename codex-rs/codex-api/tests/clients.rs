...
fn extract_metadata(req: &Request) -> Value {
    req.body
        ...
        .cloned()
        .expect("request body should contain metadata")
}
...
async fn anthropic_stream_attaches_metadata_with_session_id() -> Result<()> {
    ...
    let transport = RecordingTransport::new(state.clone());
    let client =
        codex_api::AnthropicClient::new(transport, anthropic_provider(), Arc::new(NoAuth));
...
async fn anthropic_stream_omits_metadata_without_session_id() -> Result<()> {
    ...
    let _stream = client
        .stream_request(
            anthropic_request(),
            codex_api::AnthropicOptions::default(),
        )
        .await?;
