...
fn chat_body_from_responses_request(
    ...
    let tools = chat_tools_from_responses_tools(
        request
            .tools
            .as_ref()
    ...
    for item in items {
        match item {
            ResponseItem::Message { role, content, .. } => {
                flush_pending_tool_output_images(&mut messages, &mut pending_tool_output_images);
            ...
            | ResponseItem::CustomToolCallOutput {
                ...
                let (content, images) = function_output_to_chat_content(output.body);
                messages.push(ChatMessage::Tool {
                    tool_call_id: call_id.clone(),
                    content,
                });
                if !images.is_empty() {
                    pending_tool_output_images.push(ChatContentPart::Text {
                        text: format!("Images returned by tool call `{call_id}`:"),
                    });
                    pending_tool_output_images.extend(images);
                }
            }
...
fn flush_pending_tool_output_images(
    messages: &mut Vec<ChatMessage>,
    pending_tool_output_images: &mut Vec<ChatContentPart>,
) {
    if pending_tool_output_images.is_empty() {
        return;
    }
    messages.push(ChatMessage::User {
        content: ChatContent::Parts(std::mem::take(pending_tool_output_images)),
    });
}
    ...
    for item in content {
        match item {
            ...
            ContentItem::InputImage { image, detail } => {
                // The Chat Completions API only accepts inline image URLs.
...
fn function_output_to_chat_content(
    output: FunctionCallOutputBody,
) -> (String, Vec<ChatContentPart>) {
    let items = match output {
        FunctionCallOutputBody::Text(text) => return (text, Vec::new()),
    ...
    for item in items {
        match item {
            FunctionCallOutputContentItem::InputText { text: item_text } => text.push(item_text),
            FunctionCallOutputContentItem::InputImage { image, detail } => {
                // The Chat Completions API only accepts inline image URLs.
                if let ImageReference::Inline { image_url } = image {
                    images.push(ChatContentPart::ImageUrl {
                        image_url: ChatImageUrl {
                            url: image_url,
                            detail,
                        },
    ...
    let content = if text.is_empty() && !images.is_empty() {
        "Image output is included in the following user message.".to_string()
    } else {
        text.join("\n")
    };
    (content, images)
...
mod tests {
    ...
    fn coalesces_consecutive_function_calls_into_one_assistant_message() {
        let body = body_from(request(
            vec![
                ...
                ResponseItem::FunctionCallOutput {
                    call_id: Some("call-alpha".to_string()),
                    name: None,
                    namespace: None,
                ...
                ResponseItem::Other,
                ResponseItem::FunctionCallOutput {
                    call_id: Some("call-beta".to_string()),
                    name: None,
                    namespace: None,
    ...
    #[test]
    fn preserves_images_after_parallel_tool_outputs() {
        let body = body_from(request(
            vec![
                ResponseItem::FunctionCall {
                    id: None,
                    name: "read_first".to_string(),
                    namespace: None,
                    arguments: "{}".to_string(),
                    encrypted_function_args: None,
                    call_id: "call-alpha".to_string(),
                ...
                ResponseItem::FunctionCallOutput {
                    ...
                    output: FunctionCallOutputPayload::from_content_items(vec![
                        FunctionCallOutputContentItem::InputText {
                            text: "first image".to_string(),
                        },
                        FunctionCallOutputContentItem::InputImage {
                            image: ImageReference::Inline {
                                image_url: "data:image/png;base64,alpha".to_string(),
                            },
                ...
                ResponseItem::FunctionCallOutput {
                    ...
                    output: FunctionCallOutputPayload::from_content_items(vec![
                        FunctionCallOutputContentItem::InputImage {
                            image: ImageReference::Inline {
                                image_url: "data:image/png;base64,beta".to_string(),
                            },
                            detail: None,
                        },
                    ]),
                    ...
                    "tool_calls": [
                        {
                            "id": "call-alpha",
                            "type": "function",
                            "function": {"name": "read_first", "arguments": "{}"}
                        },
                        {
                            "id": "call-beta",
                            "type": "function",
                            "function": {"name": "read_second", "arguments": "{}"}
                        }
                    ]
                },
                {"role": "tool", "tool_call_id": "call-alpha", "content": "first image"},
                {
                    "role": "tool",
                    "tool_call_id": "call-beta",
                    "content": "Image output is included in the following user message."
                },
                {
                    "role": "user",
                    "content": [
                        {"type": "text", "text": "Images returned by tool call `call-alpha`:"},
                        {
                            "type": "image_url",
                            "image_url": {"url": "data:image/png;base64,alpha", "detail": "high"}
                        },
                        {"type": "text", "text": "Images returned by tool call `call-beta`:"},
                        {
                            "type": "image_url",
                            "image_url": {"url": "data:image/png;base64,beta"}
                        }
                    ]
                }
            ])
    ...
    fn encodes_namespaced_function_call_history() {
        let body = body_from(request(
            vec![
                ...
                ResponseItem::FunctionCallOutput {
                    call_id: Some("call-namespace".to_string()),
                    name: None,
                    namespace: None,
                    output: FunctionCallOutputPayload::from_text("ok".to_string()),
                    internal_chat_message_metadata_passthrough: None,
    ...
    fn rejects_duplicate_tool_call_ids() {
        let message = chat_body_error(vec![
            ...
            ResponseItem::FunctionCallOutput {
                call_id: Some("call-dup".to_string()),
                name: None,
                namespace: None,
    ...
    fn reasoning_item_none_content(_id: &str) -> ResponseItem {
        ResponseItem::Reasoning {
            id: None,
            summary: Vec::new(),
            content: None,
