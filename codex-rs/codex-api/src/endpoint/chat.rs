...
const DEFAULT_CHAT_MAX_TOKENS: u32 = 32768;
...
mod tests {
    ...
    #[test]
    fn includes_max_tokens_in_request() {
        let body = body_from(request(
        ...
        assert_eq!(body["max_tokens"], DEFAULT_CHAT_MAX_TOKENS);
    }

    #[test]
    fn extra_body_can_override_max_tokens() {
        let mut body = json!({
            "model": "test",
            "max_tokens": DEFAULT_CHAT_MAX_TOKENS,
            "stream": true,
        ...
        let extra = HashMap::from([("max_tokens".to_string(), json!(16384))]);
