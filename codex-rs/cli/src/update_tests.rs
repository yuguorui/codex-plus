...
#[cfg(windows)]
#[test]
fn activate_release_replaces_an_existing_junction() {
    let directory = tempdir().expect("temp directory should be created");
    let standalone_root = directory.path().join("standalone");
    let releases_dir = standalone_root.join("releases");
    let bin_dir = directory.path().join("bin");
    let release_dir = releases_dir.join("202608310535-x86_64-pc-windows-msvc");
    std::fs::create_dir_all(release_dir.join("bin")).expect("release bin directory");
    std::fs::create_dir_all(&bin_dir).expect("bin directory");
    std::fs::write(release_dir.join("bin/codex++.exe"), b"first").expect("entrypoint");
    std::fs::write(
        release_dir.join("bin/codex-code-mode-host.exe"),
        b"first host",
    )
    .expect("host");
    ...
    activate_release(&layout, &release_dir).expect("release should be activated");
    std::fs::write(release_dir.join("bin/codex++.exe"), b"second").expect("new entrypoint");
    std::fs::write(
        release_dir.join("bin/codex-code-mode-host.exe"),
        b"second host",
    )
    .expect("new host");
    activate_release(&layout, &release_dir).expect("existing junction should be replaced");

    assert!(junction::exists(standalone_root.join("current")).expect("current junction"));
    assert_eq!(
        std::fs::read(bin_dir.join("codex++.exe")).expect("entrypoint"),
        b"second"
    );
    assert_eq!(
        std::fs::read(bin_dir.join("codex-code-mode-host.exe")).expect("host"),
        b"second host"
    );
}
