...
pub(crate) async fn run(action: UpdateAction) -> Result<UpdateOutcome> {
    ...
    let layout = install_layout(target)?;
...
fn install_layout(target: &str) -> Result<InstallLayout> {
    let release_dir =
        if let InstallMethod::Standalone { release_dir, .. } = &InstallContext::current().method {
            release_dir.to_path_buf()
        } else {
            #[cfg(windows)]
            {
                windows_release_dir(target)?
            }
            #[cfg(not(windows))]
            {
                let _ = target;
...
#[cfg(windows)]
fn windows_release_dir(target: &str) -> Result<PathBuf> {
    let codex_home = codex_core::config::find_codex_home()
        .context("failed to locate the Codex++ home directory")?;
    let standalone_root = codex_home.join("packages/standalone");
    let releases_dir = standalone_root.join("releases");
    let current_release =
        releases_dir.join(format!("{}-{target}", codex_cli::CODEX_CLI_DISPLAY_VERSION));
    let mut candidates = vec![current_release.into_path_buf()];
    if let Ok(current_link) = std::fs::canonicalize(standalone_root.join("current")) {
        candidates.push(current_link);
    }
    ...
    candidates
        .into_iter()
        .find(|candidate| install::validate_release(candidate, target).is_ok())
        .with_context(|| {
            format!(
                "Codex++ standalone installation could not be detected under {}",
