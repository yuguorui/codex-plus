...
fn replace_junction(target: &Path, destination: &Path) -> Result<()> {
    if destination.symlink_metadata().is_ok() {
        if junction::exists(destination).unwrap_or(false) {
            junction::delete(destination)
                .with_context(|| format!("failed to remove junction {}", destination.display()))?;
        }
        // `junction::delete` removes the reparse point but intentionally leaves
        // an empty directory behind. Remove that directory before recreating it
        // as a junction; refuse to recursively delete unexpected contents.
        std::fs::remove_dir(destination).with_context(|| {
            format!("failed to remove former junction {}", destination.display())
        })?;
    }
