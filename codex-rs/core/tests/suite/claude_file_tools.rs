...
async fn read_accepts_output_above_two_thousand_tokens() -> Result<()> {
    ...
    let mock = responses::mount_sse_sequence(
        ...
        vec![
            tool_response(
                ...
                "Read",
                &json!({
                    "file_path": file_path.inferred_native_path_string(),
                })
                .to_string(),
            ),
            done_response("resp-large-read-done", "msg-large-read-done"),
        ],
    )
    .await;
    ...
    assert_eq!(
        requests[1].function_call_output_text(READ_CALL_ID),
...
async fn rollback_removes_cached_read_authorization() -> Result<()> {
    ...
    let native_path = test.home.path().join("rollback-edit.txt");
    std::fs::write(&native_path, "alpha\nbeta\n")?;
    let file_path = native_path.to_string_lossy().to_string();
    ...
    let resumed = resume_builder.resume(&server, home, rollout_path).await?;
    submit_turn(&resumed, "Apply the edit after rollback").await?;
