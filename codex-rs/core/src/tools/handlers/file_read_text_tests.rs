use super::*;
use crate::tools::handlers::file_read_limits::DEFAULT_MAX_READ_OUTPUT_TOKENS;
...
fn zero_offset_starts_at_the_first_line_and_labels_it_zero() {
    assert_eq!(
        read_text_range_with_state("alpha\nbeta", 0, Some(1), DEFAULT_MAX_READ_OUTPUT_TOKENS,)
            .expect("read range")
            .0,
        "0\talpha"
    );
...
fn empty_and_out_of_range_reads_return_reminders() {
    assert_eq!(
        read_text_range_with_state("", 1, None, DEFAULT_MAX_READ_OUTPUT_TOKENS)
            .expect("read empty file")
            .0,
        EMPTY_FILE_REMINDER
    );
    assert_eq!(
        read_text_range_with_state("alpha\nbeta", 4, None, DEFAULT_MAX_READ_OUTPUT_TOKENS)
...
fn text_range_rejects_output_above_the_model_context_budget() {
    let output = format!("1\t{}", "x".repeat(33_000));
    let token_count = approx_token_count(&output);
    let error = read_text_range_with_state(&"x".repeat(33_000), 1, None, 2_000)
        .expect_err("oversized model output should fail");

    assert_eq!(error, read_output_too_large_error(token_count, 2_000));
}
