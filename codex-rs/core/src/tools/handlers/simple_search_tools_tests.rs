...
async fn glob_handler_supports_question_mark_and_literal_dots() {
    ...
    assert_eq!(
        ...
        json!(["src/file1.rs", "src/fileA.rs"])
    );
}
...
async fn glob_handler_caps_results_with_note() {
    ...
    assert!(
        text.contains("(Results are truncated. Consider using a more specific path or pattern.)"),
        "{text}"
    );
}
...
#[tokio::test]
async fn grep_handler_matches_claude_glob_and_type_behavior() {
    let temp_dir = tempfile::tempdir().expect("tempdir");
    std::fs::create_dir_all(temp_dir.path().join("src")).expect("mkdir");
...
async fn grep_handler_supports_content_count_and_multiline() {
    ...
    let content_result = content.code_mode_result(&ToolPayload::Function {
        arguments: "{}".to_string(),
    });
    assert_eq!(content_result["mode"], "content");
    assert_eq!(content_result["filenames"], json!(["one.txt"]));
    assert_eq!(content_result["numLines"], 2);
    ...
    assert_eq!(
        output_text(count.as_ref(), "call-Grep"),
        "one.txt:2\n\nFound 2 total occurrences across 1 file."
    );
    assert_eq!(
        count.code_mode_result(&ToolPayload::Function {
            arguments: "{}".to_string()
        })["numMatches"],
        2
    );
    ...
    let text = output_text(multiline.as_ref(), "call-Grep");
    assert!(text.contains("one.txt:1:alpha"), "{text}");
    assert!(text.contains("one.txt:2:beta"), "{text}");
}
...
#[tokio::test]
async fn grep_handler_matches_claude_hidden_and_long_line_output() {
    let temp_dir = tempfile::tempdir().expect("tempdir");
    std::fs::create_dir_all(temp_dir.path().join("src")).expect("mkdir");
    std::fs::write(temp_dir.path().join(".hidden"), "hidden needle\n").expect("write file");
    std::fs::write(
        temp_dir.path().join("src/long.txt"),
        format!("{} needle\n", "x".repeat(600)),
    )
    .expect("write file");

    let hidden = GrepHandler::new(SimpleSearchToolOptions {
        include_environment_id: false,
    })
    .handle(invocation("Grep", json!({ "pattern": "needle" }), temp_dir.path()).await)
    .await
    .expect("grep should succeed");
    assert!(
        output_text(hidden.as_ref(), "call-Grep").contains(".hidden"),
        "hidden files should be included"
    );

    let long_line = GrepHandler::new(SimpleSearchToolOptions {
        include_environment_id: false,
    })
    .handle(
        invocation(
    ...
    assert_eq!(
        output_text(long_line.as_ref(), "call-Grep"),
...
async fn grep_handler_accepts_claude_context_pagination_and_flags() {
    ...
    let text = output_text(output.as_ref(), "call-Grep");
    assert!(text.contains("one.txt:Needle"), "{text}");
    assert!(
        text.ends_with("[Showing results with pagination = limit: 2, offset: 1]"),
        "{text}"
    );
}
...
async fn grep_context_long_overrides_short_before_after_flags() {
    ...
    let text = output_text(output.as_ref(), "call-Grep");
    assert!(text.contains("one.txt:3:needle"), "{text}");
    assert!(text.contains("one.txt-2-before"), "{text}");
    assert!(text.contains("one.txt-4-after"), "{text}");
}
...
async fn grep_invalid_regex_completes_ui_item_as_failed() {
    ...
    assert!(
        item.error
            .as_ref()
            .is_some_and(|error| error.message.contains("regex parse error")),
        "{item:?}"
    );
}
...
async fn grep_handler_reports_no_matches_for_content_and_count_modes() {
    ...
    for output_mode in ["content", "count"] {
        ...
        let expected = if output_mode == "count" {
            "No matches found\n\nFound 0 total occurrences across 0 files."
        } else {
            "No matches found"
        };
        assert_eq!(output_text(output.as_ref(), "call-Grep"), expected);
    }
}
...
async fn grep_handler_maps_known_and_literal_type_filters() {
    ...
    .await;
    let Err(FunctionCallError::RespondToModel(message)) = literal_extension else {
        panic!("expected ripgrep type error");
    };
    assert!(message.contains("unrecognized file type"), "{message}");
}
...
async fn grep_handler_accepts_claude_semantic_booleans_and_numbers() {
    ...
    let text = output_text(output.as_ref(), "call-Grep");
    assert!(text.contains("one.txt:Needle"), "{text}");
    assert!(text.contains("one.txt-after"), "{text}");
}
