    ...
    async fn handle(
        ...
        if !is_pdf && is_full_text_read(&text, args.offset, args.limit) {
            tracker
                .lock()
                .await
                .record_simple_file_read(path.as_path().to_path_buf(), &text);
        }
        emit_simple_tool_completed(session.as_ref(), turn.as_ref(), &call_id, "Read", &args).await;
        Ok(boxed_tool_output(FunctionToolOutput::from_text(
            read_output(&text, args.offset, args.limit),
            Some(true),
        )))
    }
}
...
impl ToolExecutor<ToolInvocation> for WriteHandler {
    ...
    async fn handle(
        ...
        let file_change = match before {
            Some(before) => FileChange::Update {
                unified_diff: unified_diff(&before, &args.content, &path),
                move_path: None,
            },
            None => FileChange::Add {
...
fn read_output(text: &str, offset: Option<usize>, limit: Option<usize>) -> String {
...
async fn emit_file_change_begin(
    ...
    let emitter = ToolEmitter::apply_patch_with_tool_name(
