...
use crate::tools::handlers::file_read_limits::file_read_output_token_limit;
...
async fn handle_file_read(
    ...
    let max_output_tokens = file_read_output_token_limit();
    ...
    let metadata = fs
        .get_metadata(&path, GetMetadataOptions::default(), Some(&sandbox))
        .await
    ...
    let read_result: Result<(FunctionToolOutput, String, Option<String>), FunctionCallError> =
        ...
        if is_notebook {
            ...
            let result = render_notebook(&normalized, &path_summary, max_output_tokens)?;
            if !turn
                .model_info()
                .input_modalities
                .contains(&InputModality::Image)
        ...
        let (result, state_content) = if args.limit.is_some() {
            ...
            read_text_range_with_state(&normalized, offset, None, max_output_tokens)?
        };
        Ok((
    ...
    if let Some(content) = state_content {
            ...
            )
        {
            ...
            file_state_cache.remove(&turn_environment.selection.environment_id, &path);
        }
    }
    let event_ctx = ToolEventCtx::new(session.as_ref(), turn.as_ref(), &call_id, None);
    emitter
...
async fn read_file_bytes(
    ...
    fs.read_file(path, ReadFileOptions::default(), Some(sandbox))
        .await
        .map_err(|error| {
            FunctionCallError::RespondToModel(format!(
