    ...
    async fn handle(
        ...
        let ToolPayload::Function { arguments } = payload else {
            return Err(FunctionCallError::RespondToModel(
                "hashline handler received unsupported payload".to_string(),
            ));
        };
        ...
        let Some(turn_environment) =
            ...
            return Err(FunctionCallError::RespondToModel(
                "hashline is unavailable in this session".to_string(),
            ));
        };
        let cwd = turn_environment.cwd.clone();
        let path = AbsolutePathBuf::resolve_path_against_base(PathBuf::from(&args.path), &cwd);
        let sandbox = turn.file_system_sandbox_context(/*additional_permissions*/ None, &cwd);

        let output = match args.action {
            HashlineAction::Read => {
                let doc = read_document(turn_environment, &path, Some(&sandbox)).await?;
                read_output(&doc, args.anchor.as_deref(), context_lines(args.context))?
            }
            HashlineAction::Verify => {
                let doc = read_document(turn_environment, &path, Some(&sandbox)).await?;
                verify_output(&doc, require_anchor(&args)?)?
            }
            HashlineAction::Edit | HashlineAction::Insert | HashlineAction::Delete => {
                ...
                if before == after {
                    ...
                    fs.write_file(&path, after.into_bytes(), Some(&sandbox))
                        .await
                        .map_err(|error| {
                            FunctionCallError::RespondToModel(format!(
                                "hashline failed to write `{}`: {error}",
                                path.display()
                            ))
                        })?;
                    let changes = HashMap::from([(
                        ...
                        FileChange::Update {
                            unified_diff: "Edited with hashline anchors.".to_string(),
                            move_path: None,
                        },
                    )]);
                    ...
                    emitter
                        .finish(event_ctx, Ok(successful_hashline_output()), None)
                        .await?;
...
impl CoreToolRuntime for HashlineHandler {
    fn pre_tool_use_payload(&self, invocation: &ToolInvocation) -> Option<PreToolUsePayload> {
        ...
        Some(PreToolUsePayload {
            tool_name: HookToolName::new("hashline"),
            tool_input: input,
        })
    }
}

fn successful_hashline_output() -> ExecToolCallOutput {
    ExecToolCallOutput {
        exit_code: 0,
...
async fn read_document(
    ...
    let bytes = fs.read_file(path, sandbox).await.map_err(|error| {
        FunctionCallError::RespondToModel(format!(
            "hashline failed to read `{}`: {error}",
            path.display()
        ))
    })?;
    let text = String::from_utf8(bytes).map_err(|error| {
        FunctionCallError::RespondToModel(format!(
            "hashline only supports UTF-8 text files; `{}` is not valid UTF-8: {error}",
            path.display()
        ))
    })?;
...
fn require_anchor(args: &HashlineArgs) -> Result<&str, FunctionCallError> {
    args.anchor.as_deref().ok_or_else(|| {
        FunctionCallError::RespondToModel(format!(
            "hashline action `{}` requires `anchor`",
...
fn require_content(args: &HashlineArgs) -> Result<&str, FunctionCallError> {
    args.content.as_deref().ok_or_else(|| {
        FunctionCallError::RespondToModel(format!(
            "hashline action `{}` requires `content`",
            action_name(args.action)
        ))
    })
}
...
fn read_output(
    ...
    let (start, end) = if let Some(anchor) = anchor {
        let resolved = resolve_anchor(doc, parse_anchor(anchor)?)?;
        context_window(doc.lines.len(), resolved.index, resolved.index, context)
    } else {
        (0, doc.lines.len())
    };
    Ok(format_lines(doc, start, end, None))
}
...
fn apply_mutation(
    ...
    match args.action {
        ...
        HashlineAction::Insert => {
            ...
            let content = content.ok_or_else(|| {
                FunctionCallError::RespondToModel(
                    "hashline action `insert` requires `content`".to_string(),
                )
            })?;
            let resolved = resolve_anchor(doc, parse_anchor(anchor)?)?;
        ...
        HashlineAction::Read | HashlineAction::Verify => Err(FunctionCallError::RespondToModel(
            "internal hashline mutation dispatch error".to_string(),
        )),
    }
}
...
fn resolve_anchor_or_range(
    ...
    if let Some((left, right)) = anchor_or_range.split_once("..") {
        ...
        let start = resolve_anchor(doc, parse_anchor(left)?)?;
        let end = resolve_anchor(doc, parse_anchor(right)?)?;
        if start.index > end.index {
            return Err(FunctionCallError::RespondToModel(format!(
                "hashline range `{anchor_or_range}` resolves backwards"
            )));
        }
        return Ok((start.index, end.index));
    }
    let resolved = resolve_anchor(doc, parse_anchor(anchor_or_range)?)?;
    Ok((resolved.index, resolved.index))
}

fn parse_anchor(anchor: &str) -> Result<Anchor, FunctionCallError> {
...
fn invalid_anchor(anchor: &str) -> FunctionCallError {
    FunctionCallError::RespondToModel(format!(
        "invalid hashline anchor `{anchor}`; expected `12:ab`, `ab`, or `12:ab..15:ef`"
    ))
}
    ...
    match anchor {
        Anchor::Hash(hash) => match index[hash as usize].as_slice() {
            ...
            matches => Err(FunctionCallError::RespondToModel(format!(
                "hashline hash `{}` is ambiguous; it appears on lines {}",
                format_short_hash(hash),
