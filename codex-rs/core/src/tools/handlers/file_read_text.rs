...
pub(super) fn read_text_range_with_state(
    ...
    let output = selected
        ...
        .collect::<Vec<_>>()
        .join("\n");
    validate_read_output_size(&output, max_output_tokens)?;
...
pub(super) async fn read_text_range_stream(
    ...
    let mut stream = match fs.read_file_stream(path, sandbox).await {
        ...
        Err(error) if error.kind() == io::ErrorKind::Unsupported => {
            return read_text_range_buffered(
                fs,
                path,
                sandbox,
                path_display,
    ...
    let mut selected_output_bytes = 0usize;
    ...
    'read_chunks: loop {
        ...
        for (index, byte) in chunk.iter().enumerate() {
            ...
            if (start_index..end_index).contains(&current_index) {
                ...
                let line_number = offset.saturating_add(selected_lines.len());
                push_streamed_line(
                    &mut selected_lines,
                    &mut selected_line,
                    &mut selected_output_bytes,
                    current_index == 0,
                    line_number,
                    max_output_tokens,
                )?;
        ...
        if (start_index..end_index).contains(&current_index) {
            ...
            let selected_bytes_with_line =
                selected_output_bytes.saturating_add(formatted_line_bytes(
                    offset.saturating_add(selected_lines.len()),
                    selected_line.len(),
                    !selected_lines.is_empty(),
                ));
            let token_count = approx_tokens_for_bytes(selected_bytes_with_line);
            if token_count > max_output_tokens {
                return Err(read_output_too_large_error(token_count, max_output_tokens));
            }
        }
    }
...
async fn read_text_range_buffered(
    ...
    read_text_range_with_state(&normalized, offset, limit, max_output_tokens)
}

fn push_streamed_line(
    selected_lines: &mut Vec<String>,
    line: &mut Vec<u8>,
    selected_output_bytes: &mut usize,
    strip_bom: bool,
    line_number: usize,
    max_output_tokens: usize,
    ...
    *selected_output_bytes = selected_output_bytes.saturating_add(formatted_line_bytes(
        line_number,
        text.len(),
        !selected_lines.is_empty(),
    ));
    let token_count = approx_tokens_for_bytes(*selected_output_bytes);
    if token_count > max_output_tokens {
        return Err(read_output_too_large_error(token_count, max_output_tokens));
    }
...
fn validate_read_output_size(
    output: &str,
    max_output_tokens: usize,
) -> Result<(), FunctionCallError> {
    let token_count = approx_token_count(output);
    if token_count > max_output_tokens {
        return Err(read_output_too_large_error(token_count, max_output_tokens));
    }
    Ok(())
...
fn approx_tokens_for_bytes(bytes: usize) -> usize {
    usize::try_from(approx_tokens_from_byte_count(bytes)).unwrap_or(usize::MAX)
}

fn formatted_line_bytes(line_number: usize, line_bytes: usize, has_previous: bool) -> usize {
    line_number
        .to_string()
        .len()
        .saturating_add(1)
        .saturating_add(line_bytes)
        .saturating_add(usize::from(has_previous))
}

fn read_output_too_large_error(token_count: usize, max_output_tokens: usize) -> FunctionCallError {
    FunctionCallError::RespondToModel(format!(
        "File content ({token_count} tokens) exceeds maximum allowed tokens ({max_output_tokens}). Use offset and limit parameters to read specific portions of the file, or search for specific content instead of reading the whole file."
    ))
}
