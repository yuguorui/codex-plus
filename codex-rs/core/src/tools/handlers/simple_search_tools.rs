...
impl ToolExecutor<ToolInvocation> for GlobHandler {
    ...
    async fn handle(
        ...
        let output_result = async {
            ...
            {
                return Ok(render_paths(
                    paths.into_iter().map(|path| display_path(&path, &cwd)),
                ));
            }
            ...
            Ok(render_paths(
                files.into_iter().map(|file| file.display_path),
            ))
        }
        .await;
...
fn render_paths(paths: impl IntoIterator<Item = String>) -> String {
...
fn render_grep_lines_with_args(lines: Vec<String>, args: &GrepArgs) -> String {
