...
impl ToolExecutor<ToolInvocation> for GlobHandler {
    ...
    async fn handle(
        ...
        let output_result = async {
            ...
            let mut files = collect_files(turn_environment, &root, &cwd, Some(&sandbox)).await?;
            let matcher = GlobMatcher::new(&args.pattern)?;
            ...
            Ok(render_paths(
                files.into_iter().map(|file| file.display_path).collect(),
            ))
        }
        .await;
...
fn render_paths(paths: Vec<String>) -> String {
    if paths.is_empty() {
    ...
    if total > MAX_GLOB_RESULTS {
        output.push_str(&format!(
            "\n\nResults truncated to {MAX_GLOB_RESULTS} of {total} files."
        ));
    }
    output
}
...
fn render_grep_matches_with_args(matches: &[GrepFileMatch], args: &GrepArgs) -> String {
    if matches.is_empty() {
    ...
    if end < total {
        ...
        output.push_str(&format!("Results truncated. Use offset={end} to continue."));
    }
    output
}
