...
fn output_text(output: &dyn ToolOutput, call_id: &str) -> String {
    let FunctionCallOutputBody::Text(text) = output_body(output, call_id) else {
        panic!("expected text output");
    };
    text
}

fn output_body(output: &dyn ToolOutput, call_id: &str) -> FunctionCallOutputBody {
...
#[tokio::test]
async fn read_returns_image_content_item() {
    let temp_dir = tempfile::tempdir().expect("tempdir");
    let image_bytes = [0x89, b'P', b'N', b'G', b'\r', b'\n', 0x1a, b'\n'];
    std::fs::write(temp_dir.path().join("sample.png"), image_bytes).expect("write image");
    ...
    let FunctionCallOutputBody::ContentItems(items) = output_body(output.as_ref(), "call-Read")
    else {
...
#[tokio::test]
async fn read_rejects_image_extension_with_non_image_bytes() {
    let temp_dir = tempfile::tempdir().expect("tempdir");
    std::fs::write(temp_dir.path().join("sample.png"), b"not a png").expect("write image");

    let Err(FunctionCallError::RespondToModel(message)) = ReadHandler::new(SimpleFileToolOptions {
        include_environment_id: false,
    })
    .handle(
        invocation(
    ...
    .await
        ...
        panic!("expected image read error");
    };

    assert!(
        message.contains("file contents do not match image type image/png"),
        "{message}"
    );
}

#[tokio::test]
async fn read_returns_file_unchanged_stub_for_repeated_full_read() {
    let temp_dir = tempfile::tempdir().expect("tempdir");
    ...
    .handle(context.invocation("Read", json!({ "file_path": "sample.txt" })))
    .await
    .expect("read should succeed");
    ...
    assert_eq!(
        ...
        json!({
            "type": "file_unchanged",
            "file": {
                "filePath": temp_dir.path().join("sample.txt").display().to_string(),
            },
        })
    );
}
...
#[tokio::test]
async fn read_after_edit_returns_updated_content_instead_of_file_unchanged() {
    let temp_dir = tempfile::tempdir().expect("tempdir");
    std::fs::write(temp_dir.path().join("sample.txt"), "one\n").expect("write file");
    let context = ToolTestContext::new(temp_dir.path()).await;

    context.read("sample.txt").await;
    let edit_output = EditHandler::new(SimpleFileToolOptions {
        include_environment_id: false,
    })
    ...
    let edit_result = edit_output.code_mode_result(&ToolPayload::Function {
        arguments: "{}".to_string(),
    });
    assert_eq!(
        edit_result["filePath"],
        json!(temp_dir.path().join("sample.txt").display().to_string())
    );
    assert_eq!(edit_result["oldString"], "one");
    assert_eq!(edit_result["newString"], "two");
    assert_eq!(edit_result["replaceAll"], false);
    assert_eq!(edit_result["structuredPatch"][0]["oldStart"], 1);
    let output = ReadHandler::new(SimpleFileToolOptions {
        include_environment_id: false,
    })
    .handle(context.invocation("Read", json!({ "file_path": "sample.txt" })))
    ...
    assert_eq!(output_text(output.as_ref(), "call-Read"), "     1\ttwo");
}

#[tokio::test]
async fn read_after_write_returns_updated_content_instead_of_file_unchanged() {
    let temp_dir = tempfile::tempdir().expect("tempdir");
    std::fs::write(temp_dir.path().join("sample.txt"), "one\n").expect("write file");
    let context = ToolTestContext::new(temp_dir.path()).await;

    context.read("sample.txt").await;
    let write_output = WriteHandler::new(SimpleFileToolOptions {
        include_environment_id: false,
    })
    .handle(context.invocation(
        "Write",
    ...
    let write_result = write_output.code_mode_result(&ToolPayload::Function {
        arguments: "{}".to_string(),
    });
    assert_eq!(write_result["type"], "update");
    assert_eq!(
        write_result["filePath"],
        json!(temp_dir.path().join("sample.txt").display().to_string())
    );
    assert_eq!(write_result["originalFile"], "one\n");
    assert_eq!(write_result["structuredPatch"][0]["newStart"], 1);
    let output = ReadHandler::new(SimpleFileToolOptions {
        include_environment_id: false,
    })
    .handle(context.invocation("Read", json!({ "file_path": "sample.txt" })))
...
async fn bash_executes_command_in_turn_cwd() {
    ...
    let (_begin, _end) = bash_exec_events(&rx_event).await;
}
