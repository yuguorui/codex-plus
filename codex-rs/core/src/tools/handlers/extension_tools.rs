...
mod tests {
    ...
    async fn passes_step_node_repl_flag_and_scoped_turn_item_emitter_to_extension_call() {
        ...
        let expected_model = settings.model_info.slug.clone();
        let expected_truncation_policy = settings.model_info.truncation_policy.into();
        let mut expected_agent_config = (*turn.config).clone();
        expected_agent_config.model = Some(expected_model.clone());
        expected_agent_config.model_reasoning_effort = settings.reasoning_effort().cloned();
        expected_agent_config.model_reasoning_summary = Some(settings.reasoning_summary);
        expected_agent_config
            .service_tier
            .clone_from(&settings.service_tier);
        expected_agent_config.permissions.approval_policy =
            codex_config::Constrained::allow_only(settings.approval_policy());
        expected_agent_config.approvals_reviewer = settings.approvals_reviewer();
