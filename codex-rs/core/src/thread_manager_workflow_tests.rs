...
async fn rollout_independent_fresh_subagents_use_the_owning_registry() {
    ...
    let first_control = first_owner
        .thread
        .session
        .services
        .local_agent_runtime
        .control(first_owner.thread.session.session_id());
    let second_control = second_owner
        .thread
        .session
        .services
        .local_agent_runtime
        .control(second_owner.thread.session.session_id());
    ...
    assert_eq!(
        first_agent
            ...
            .services
            .local_agent_runtime
            .control(first_agent.thread.session.session_id())
...
async fn fresh_workflow_subagent_persists_ownership_edge_and_close_status() {
    ...
    let owner_control = owner
        .thread
        .session
        .services
        .local_agent_runtime
        .control(owner.thread.session.session_id());
    ...
    for _ in 0..2 {
        ...
        let status = updates
            .next()
            .await
            .expect("completed workflow agent should publish a snapshot")
            .expect("completed workflow agent should publish a snapshot")
            .status()
            .cloned()
