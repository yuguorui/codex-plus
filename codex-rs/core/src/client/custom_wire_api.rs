...
pub(super) async fn stream_chat_api(
    ...
    loop {
        ...
        let mut request = session.client.build_responses_request(
            prompt,
            model_info,
            effort.clone(),
            summary,
            service_tier.clone(),
            responses_metadata,
            is_internal_metadata_destination(&client_setup.api_provider),
        )?;
        let interceptors = crate::model_request::prepare(
            &session.client.request_contributors,
            &session.client.state.thread_id.to_string(),
            &model_info.slug,
            ...
            Err(ApiError::Transport(
                ...
                pending_retry = PendingUnauthorizedRetry::from_recovery(
                    handle_unauthorized(
                        ...
                        session.client.event_sender.as_ref(),
